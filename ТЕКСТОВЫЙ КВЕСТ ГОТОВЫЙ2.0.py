...
if qfirst == 'y':
    ...
    if qsec == 'y':
        ...
        if qsrd == 'x':
            ...
            print ('нечто зашло в комнату и оказалось что это был напившийся отчим который хочет использовать вас как закуску,он ходил по комнате в поисках вас и у вас осталось чуть больше минуты чтобы что нибудь предпренять,вы открывайте телефон открыть папку игры(x)позвонить спасателям (y)начать стрим на твиче (z)')
            qfth =input('введите вариант ответа')
            if qfth == 'x':
                qsth=input('выберете игру бравл старс-x райз оф киндомс-y рэйд шэдоу легендс-z')
            elif qfth== 'y':
                print('ты живешь в россии-плохая концовка')
            elif qfth == 'z':
                print ('вы ачали стрим на твиче но в вашу комнату ворвался отчим вы закричали ненадо дядя а далше следовао смачное яяяяяях-плохая концовка')
